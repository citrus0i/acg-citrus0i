...
auto sample_hemisphere(
  ...
  //pdf = (local_to_world_vector_transformation(nrm) * dir_loc).dot(nrm) / float(M_PI);  //both worked
  pdf = z / float(M_PI);    //z=cos(theta)=nrm.dot(omega_i)
  ...
  if (bvhnodes[i_bvhnode].is_leaf()) { // this is leaf node
    ...
    if(bvhnodes[i_bvhnode].intersect_bv(ray_org, ray_dir)){
        search_collision_in_bvh(
                ...
                i_node_right, // root node index
                ray_org, ray_dir, tri2vtx, vtx2xyz, bvhnodes);
        search_collision_in_bvh(
                is_hit, hit_depth, hit_pos, hit_normal,
                i_node_left, // root node index
                ray_org, ray_dir, tri2vtx, vtx2xyz, bvhnodes);
    }
  }
}
