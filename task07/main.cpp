  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = hit1_rad * hit0_brdf * fmax(0.0f, hit0_refl.dot(hit0_normal)) / hit0_pdf / nsample;
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float w_sum = hit0_pdf_brdf_sample + hit0_pdf_light_sample;
        float w_brdf = hit0_pdf_brdf_sample / w_sum;
        float rad = w_brdf * hit1_rad * hit0_brdf * fmax(0.0f, hit0_refl.dot(hit0_normal)) / hit0_pdf_brdf_sample / num_half_sample;
      ...
      for (int isample = 0; isample < nsample / 2; ++isample) {
        ...
        float w_sum = hit0_pdf_brdf_sample + hit0_pdf_light_sample;
        float w_light = hit0_pdf_light_sample / w_sum;
        float rad = w_light * hit1_rad * hit0_brdf * fmax(0.0f, hit0_refl.dot(hit0_normal)) / hit0_pdf_light_sample / num_half_sample;
        img_mis[ih * img_width + iw] += rad;
      }
    }
  }
